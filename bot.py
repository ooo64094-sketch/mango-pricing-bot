...
async def extract_visible_iqd_price(page):
    ...
    if len(candidates) > 1:
        ...
        if (
            ...
            and abs(best["distance"] - second["distance"]) < 120
        ):
            return None
...
def find_turkey_links(ref_code: str, original_url: str = None):
    ...
    if (
        original_url
        and original_url.startswith("http")
        and "shop.mango.com/tr/" in original_url
    ):
...
async def check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "ارسل رابط Mango تركيا أو الريفيرانس بعد /check"
        )
        return
    ...
    try:
        ...
        turkey_links = find_turkey_links(
            ref_code,
        ...
        result = calculate_quote(
            turkey_data["price"], iraq_data["price_iqd"]
        )
